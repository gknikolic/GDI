...
#pragma region 2016KOL1Bilijar
#pragma region  code
void GDIView::DrawStick(CDC* draw, int w)
{
}

void GDIView::DrawHole(CDC* draw, int size)
{
    draw->SelectObject(new CBrush(RGB(0, 0, 0)));
    CRect rect(0, 0, size, size);
    draw->Ellipse((LPCRECT(rect)));
}

void GDIView::DrawBorder(CDC* draw, CRect rect, int w)
{
    wood->Draw(draw, CRect(0, 0, wood->Width(), wood->Height()), rect);
}

void GDIView::DrawTable(CDC* draw, CRect rect)
{
    table->Draw(draw, CRect(0, 0, table->Width(), table->Height()), rect);
}

void GDIView::DrawBall(CDC* draw, int w)
{
    draw->SelectObject(new CPen(PS_DEFAULT, 3, RGB(100, 0, 0)));
    draw->SelectObject(new CBrush(RGB(200, 0, 0)));
    CRect rect(0, 0, w, w);
    draw->Ellipse(rect);
    draw->SelectObject(new CPen(PS_DEFAULT, 1, RGB(200, 200, 200)));
    draw->SelectObject(new CBrush(RGB(255, 255, 255)));
    CRect rect1(0, 0, w / 3, w / 3);
    Translate(draw, 10, 10, false);
    draw->Ellipse(rect1);
    Translate(draw, -10, -10, false);
}


void GDIView::DrawHoles(CDC* draw, CRect rect, int size)
{
    Translate(draw, rect.top - size / 2, rect.left - size / 2, false);
    DrawHole(draw, size);
    Translate(draw, +rect.Width() / 2, 0, false);
    DrawHole(draw, size);
    Translate(draw, rect.Width() / 2, 0, false);
    DrawHole(draw, size);
    Translate(draw, 0, rect.Height(), false);
    DrawHole(draw, size);
    Translate(draw, -rect.Width() / 2, 0, false);
    DrawHole(draw, size);
    Translate(draw, -rect.Width() / 2, 0, false);
...
void GDIView::Draw(CDC* draw)
{
	DrawBorder(memDC, cRect, 1);
	CRect tableRect(cRect.left + 50, cRect.top + 50, cRect.right - 50, cRect.bottom - 50);
	DrawTable(memDC, tableRect);
	//DrawHoles(memDC, tableRect, 30);
